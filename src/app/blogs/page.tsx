...
import { FiEdit3, FiExternalLink, FiPlus, FiSearch, FiClock, FiTag, FiChevronRight, FiBookmark, FiTrendingUp, FiStar } from 'react-icons/fi';
...
// Predefined star positions with consistent values
const STARS = [
...
export default function BlogPage({ params }: { params?: { category?: string } }) {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        ...
        const categories = [...new Set(data.map((blog: Blog) => blog.category))];
        if (categories.length > 0) {
          setActiveCategory(params?.category || categories[0]);
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    // Apply search across all blogs by title only
    if (searchQuery) {
      results = blogs.filter(blog =>
        blog.title.toLowerCase().includes(searchQuery.toLowerCase())
  )}

    // If no search, apply category filtering
    if (!searchQuery && activeCategory) {
      results = blogs.filter(blog => blog.category === activeCategory);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 relative overflow-hidden">
      {/* Animated background stars - only render on client */}
      ...
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-sm border-b border-gray-200 shadow-sm">
        <div className="container mx-auto px-4 py-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex-1 min-w-0">
              <h1 className="text-3xl md:text-4xl font-serif font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-800 to-purple-800 tracking-tight">
                {activeCategory ? 
                  `${activeCategory.charAt(0).toUpperCase() + activeCategory.slice(1)} Chronicle` : 
                  'The Modern Gazette'}
              </h1>
              <p className="text-gray-600 mt-1 font-serif italic text-sm md:text-base">
                {activeCategory ? 
                  `Curated perspectives on ${activeCategory}` : 
          ...
          <div className="mt-4 flex overflow-x-auto scrollbar-hide space-x-2 pb-2">
      ...
      <main className="container mx-auto px-4 py-8 relative z-10">
        {error ? (
          ...
          <>
            {/* Hero Featured Story - Magazine Cover Style */}
            ...
            {/* Trending Now Section - Magazine Style */}
            ...
            <div className="flex flex-col lg:flex-row gap-8 mb-16">
              ...
              {/* Sidebar Column - Magazine Style */}
              <div className="lg:w-1/3">
                {/* Stellar Content Section */}
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl p-6 mb-8 shadow-sm border border-indigo-100 relative overflow-hidden">
                  {/* Only render floating stars on client */}
                  {isClient && (
                    <>
                      <motion.div 
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 relative overflow-hidden">
                  {/* Only render animated constellation on client */}
                  {isClient && (
